...
- **Agent 配置下载/上传**：WebDAV 只下载/上传 Cursor 侧 Skill / Command / Rule / Hook；Codex 由本机从 Cursor 转换生成
...
4. 在「Agent 配置下载/上传」中下载或合并 Cursor 资源（Skill / Command / Rule / Hook），应用到 Cursor 后再按需「一键转换」到 Codex / Open Code
...
| `{remotePath}/hooks.zip` | Cursor Hook（`hooks.json` + `hooks/`） |

**下载**把压缩包解压后覆盖对应缓存或清单；**合并**先解压再与本机合并（清单三路合并；资源覆盖同名、保留本地多余项）。自动拉取使用合并，避免定时整包覆盖本机。旧版 `catalog.json` 与 `{skills\|commands\|rules\|hooks}/cursor/` 仅在对应 zip 不存在时回退读取，上传不再写入目录树。
...
| 角色 | 路径 |
|------|------|
| WebDAV（权威） | `{remotePath}/{skills\|commands\|rules\|hooks}.zip` |
| 本机缓存 | `~/.mcp-hub/skills/cursor/` 或 `~/.mcp-hub/agent-resources/{commands\|rules\|hooks}/cursor/` |
| 部署 Cursor | `~/.cursor/skills/`、`~/.cursor/commands/`、`~/.cursor/rules/`、`~/.cursor/hooks.json` 与 `~/.cursor/hooks/` |
| 本机 Codex（转换产物） | `~/.codex/skills/`、`~/.codex/AGENTS.md`、`~/.codex/hooks.json` 与 `~/.codex/hooks/` |

每个 Skill 是一个含 `SKILL.md` 的子文件夹。下载为压缩包解压覆盖缓存；合并为解压后覆盖同名、不删除缓存多余项。应用到 Cursor 仍为全量镜像。跳过 `.` 开头目录。

**迁移说明**：若旧远端仍有 `catalog.json` 或 `{remotePath}/{skills\|commands\|rules\|hooks}/cursor/`（以及历史 `codex/` 目录），新版本仅在对应 zip 不存在时回退读取目录树/`catalog.json`；上传只覆盖固定名 zip，不会自动删除旧目录。可手工清理，或以 Cursor 为准重新「上传全部」。本机 Codex / Open Code 请用「一键转换」从 Cursor 生成。
...
| Cursor Hook | `{remotePath}/hooks.zip` | `~/.cursor/hooks.json` 与 `~/.cursor/hooks/` |
| Codex Hook | （不通过 WebDAV） | 由 Cursor Hook 转换写入 `~/.codex/hooks.json` 与 `~/.codex/hooks/` |

Cursor Command 使用 Markdown 文件。Codex 暂无与 Cursor 全局 Command 对等的入口，因此 Command 不下载/上传、不转换到 Codex。

Hook 压缩包内是 `hooks.json` 与 `hooks/` 脚本目录；应用到 Cursor 时只覆盖这两处，不会删除 `mcp.json` 等其它配置。Open Code 使用 TypeScript 插件而非 `hooks.json`，因此 Hook 不一键转换到 Open Code。
...
| Hook | `~/.cursor/hooks.json` 与 `~/.cursor/hooks/` | `~/.codex/hooks.json` 与 `~/.codex/hooks/` | 脚本目录镜像；事件改成 Codex 三层结构（如 `beforeShellExecution` → `PreToolUse` + matcher `Bash`）；相对路径改写为 `~/.codex/hooks/`；必要时写入 `features.codex_hooks = true`。Open Code 无对等 `hooks.json` |

界面在推荐流程提供「一键转换全部目标」，并在 Skill / Command / Rule / Hook 卡片各提供「一键转换」。转换产物以本机 Cursor 为准全量对齐附件后，再写成各客户端格式：Skill 整包镜像（多余文件与多余包会删除），Codex 写入 `agents/openai.yaml`，OpenCode 改写 `SKILL.md` 为其所识别的 frontmatter；Rule 整文件覆盖 `AGENTS.md`；OpenCode Command 删除 Cursor 中已不存在的命令文件；Hook 写入 Codex `hooks.json` 并镜像脚本。OpenCode **不读取、合并或覆盖** 其余 JSON/JSONC 配置。Skill 的正文与 `scripts/`、`references/` 两边共用。Codex `allow_implicit_invocation` **每次以 Cursor 的 `disable-model-invocation` 为准覆盖**（`true` → 禁止隐式调用；缺省则允许），不保留 Codex 旧值。`short_description` 从 description 生成，长度 25–64。OpenCode 将同一字段映射为 `metadata.opencode/autoinvoke`。
...
| `~/.mcp-hub/agent-resources/` | Command / Rule / Hook WebDAV 本地缓存 |
